...
CREATE POLICY "uso_voucher_insert_policy" ON uso_voucher AS PERMISSIVE
    ...
    WITH CHECK (
        ...
        NOT EXISTS (
            ...
            WHERE uv.usuario_id = uso_voucher.usuario_id
            AND uv.tipo_refeicao_id = uso_voucher.tipo_refeicao_id
        ...
        (
            SELECT COUNT(*)
            FROM uso_voucher uv
            JOIN usuarios u ON u.id = uso_voucher.usuario_id
            WHERE uv.usuario_id = uso_voucher.usuario_id
            AND u.turno_id = (SELECT turno_id FROM usuarios WHERE id = uso_voucher.usuario_id)
            AND DATE(uv.usado_em) = CURRENT_DATE
        ) < 2
        AND
        ...
        EXISTS (
            ...
            JOIN tipos_refeicao tr ON tr.id = uso_voucher.tipo_refeicao_id
            WHERE u.id = uso_voucher.usuario_id
            AND t.ativo = true
            AND tr.ativo = true
