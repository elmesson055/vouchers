...
CREATE POLICY "vouchers_descartaveis_select_policy" ON vouchers_descartaveis
    ...
    USING (
        ...
        EXISTS (
            ...
            WHERE auth.uid() = u.id
            AND raw_user_meta_data->>'role' IN ('admin', 'gestor')
        )
    );
...
'Permite visualizar vouchers válidos para usuários autenticados e anônimos. Admins e gestores podem ver todos.';
